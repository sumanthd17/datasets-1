...
flags.DEFINE_string('save_path', None,
                    'Path to save generated data to.')
flags.DEFINE_integer('num_graphs', 12,
                     'Number of graphs to create.')
flags.DEFINE_integer('seed', 7,
...
def generate_dummy_graphs(num_graphs: int) -> List[ArrayDict]:
  ...
  return [generate_dummy_graph(num_nodes, num_edges)
          for (num_nodes, num_edges) in zip(all_num_nodes, all_num_edges)]


def generate_dummy_graph(num_nodes: int,
                         num_edges: int,
                         node_feature_dim: int = 9,
                         edge_feature_dim: int = 3,
                         labels_dim: int = 128) -> ArrayDict:
...
def save_to_path(path: ReadWritePath,
  ...
  for column, column_vals in graphs_dict.items():
    ...
    column_df.to_csv(column_save_name,
